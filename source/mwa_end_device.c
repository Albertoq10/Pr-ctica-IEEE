...
void intToStr(uint8_t number, char* buffer) {
    uint8_t i = 0;
    uint8_t j;
    char temp[3]; // Tamaño suficiente para valores de 0 a 99

    // Convierte el número a caracteres
    do {
        temp[i++] = (number % 10) + '0';
        number /= 10;
    } while (number > 0 && i < sizeof(temp));

    // Invierte los caracteres en temp y cópialos en el buffer
    for (j = 0; j < i; j++) {
        buffer[j] = temp[i - j - 1];
    }
...
void SendCounterPacket()
{
    ...
    if( (mcPendingPackets < mDefaultValueOfMaxPendingDataPackets_c) && (mpPacket == NULL) )
    {
        ...
        if(mpPacket != NULL)
        {
			...
			char message[16] = "Counter:";
			intToStr(GetContador(), message + 8);  // Coloca el número después de "Counter:"

			// Copia el mensaje construido al buffer del mensaje
           ...
           /* Create the header using coordinator information gained during
            ...
            /* Don't use security */
            mpPacket->msgData.dataReq.securityLevel = gMacSecurityNone_c;
            NWK_MCPS_SapHandler(mpPacket, macInstance);//realiza el data request
...
{
#if gKBD_KeysCount_c > 0 //se brinca directamente a default
    switch ( events ) 
    { 
    ...
    case gKBD_EventSW4_c:
    	if(associate_flag != 0){//hasta que se asoacia
    		if (events == 1){
    		    switch_pressed(2);
